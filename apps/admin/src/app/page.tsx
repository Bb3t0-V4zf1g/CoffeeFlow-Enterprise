...
export default async function Home() {
    ...
    const lowStockItems = snapshot.inventory
        ...
        .sort(
            (left, right) =>
                left.current_quantity - left.minimum_quantity -
    ...
    const readyToday = todayOrders.filter((order) => order.status === "ready")
        .length;
    const servedToday = todayOrders.filter((order) => order.status === "served")
        .length;
    const criticalInventory = lowStockItems.length;
    const activeProducts = snapshot.products.filter((product) => product.is_active)
        .length;
    const averageTicket =
        todayOrders.length === 0 ? 0 : Math.round(todayRevenue / todayOrders.length);
            ...
            summary={
                <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-600">
                        {todayOrders.length} pedidos hoy · {criticalInventory} alertas de inventario
                    </p>
                    <p className="text-xs text-slate-500">
                        Último pedido: {lastOrder ? lastOrder.id.slice(0, 8) : "sin datos"}
                    </p>
                </div>
            }
        >
                        ...
                        {recentOrders.map((order) => (
                            ...
                            >
                                ...
                                <p className="mt-3 text-sm text-slate-700">
                                    {order.summary || `${order.itemCount} artículos`}
                                </p>
                                <div className="mt-3 flex items-center justify-between text-sm">
                                    <span className="text-slate-500">
                                        {order.notes ? `Nota: ${order.notes}` : "Sin notas"}
                                    </span>
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                        ...
                                        <div className="text-right text-sm">
                                            ...
                                            <p className="text-slate-500">
                                                {product.orders} pedidos · {product.revenueText}
                                            </p>
                                        </div>
                                    </div>
                                </article>
            ...
            <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
                <Panel
                    title="Inventario crítico"
                    subtitle="Insumos que ya requieren revisión o reabasto."
                >
                    <div className="space-y-3">
                                    ...
                                    >
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">
                                                {item.current_quantity} {item.unit} · mínimo {item.minimum_quantity}
                                            </p>
                                        </div>
                                        <ToneChip tone="rose">
                ...
                >
                    <div className="grid gap-3 md:grid-cols-2">
                        ...
                        >
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                                Revisión rápida de pedidos visibles para clientes.
                            </p>
                        </Link>
                        <Link
